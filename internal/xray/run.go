...
func getCoreURL() (url, filename string, isTarGz bool, err error) {
	switch runtime.GOOS {
	case "windows":
		return fmt.Sprintf("https://github.com/XTLS/Xray-core/releases/download/v%s/Xray-windows-64.zip", releaseVersion),
			"xray.exe", false, nil
	case "linux":
		switch runtime.GOARCH {
		case "amd64":
			return fmt.Sprintf("https://github.com/XTLS/Xray-core/releases/download/v%s/Xray-linux-64.zip", releaseVersion),
				"xray", false, nil
		case "arm64":
			return fmt.Sprintf("https://github.com/XTLS/Xray-core/releases/download/v%s/Xray-linux-arm64.zip", releaseVersion),
				"xray", false, nil
		}
	}
...
func downloadAndExtract() (string, error) {
	...
	if _, err := os.Stat(dest); err == nil {
		logger.Log(logger.INFO, "Xray core already exists")
		return dest, nil
	}
	...
	logger.Log(logger.INFO, "Downloading Xray core from "+url)
	...
	logger.Log(logger.INFO, "Xray core prepared at "+execPath)
	return execPath, nil
}
...
func extractZipAll(zipPath, destDir string) (string, error) {
	...
	for _, f := range r.File {
		...
		if base == "xray.exe" || base == "xray" {
			execPath = outPath
		}
	}

	if execPath == "" {
		return "", errors.New("Xray executable not found in zip")
	}
	return execPath, nil
}
...
func extractTarGzAll(tarGzPath, destDir string) (string, error) {
	...
	if execPath == "" {
		return "", errors.New("Xray executable not found in tar.gz")
	}
	return execPath, nil
}
...
func RunXrayStream(ctx context.Context, args []string, callback func(string)) bool {
	...
	if err := cmd.Start(); err != nil {
		callback("Failed to start Xray: " + err.Error())
		return false
	}
...
func KillXray() {
	...
	if currentProc != nil && currentProc.Process != nil {
		...
		logger.Log(logger.INFO, "Xray process killed")
		currentProc = nil
	}
}
